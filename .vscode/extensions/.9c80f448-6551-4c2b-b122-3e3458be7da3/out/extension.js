...
const date_inline_and_chat = '2024-06-01'
...
async function streamResponse({
	...
	try {
		...
		for await (const chunk of response) {
			...
			const replaceRange = new vscode.Range(startPosition, currentEndPosition);

			await suggestionManager.updateSuggestionText(
				suggestionId,
				accumulatedText,
				replaceRange
			);
		...
		const finalRange = new vscode.Range(startPosition, currentEndPosition);
		await suggestionManager.decorateFinalLine(suggestionId, finalRange);

		// stream answer don 
		globalSuggestionId = suggestionId
	} catch (error) {
...
function activate(_) {	
	...
	let selectedIcon = 'oldicon'
	const new_icon_date = '2024-11-07'
	const conditionForNewIcon = installed_date > dateToMilliseconds(new_icon_date)
	if (conditionForNewIcon){
		selectedIcon = 'changedicon'
	}
	vscode.commands.executeCommand('setContext', 'blackboxIcon', selectedIcon)
	...
	if (
		...
		function updateDecoration(editor) {
			...
			let decorationText = `Chat (${shortcutKey}) / Edit (${shorcutForInlineChat})`
			...
			const decorationOptions = {
				...
				renderOptions: {
					after: {
						...
						backgroundColor: new vscode.ThemeColor('editor.background'), // Background color
						borderColor: '#b0b0b0', // Lighter gray border color
						border: '0.1px solid', // Border style
						textDecoration: 'none',
						cursor: 'pointer',
  ...
  vscode.commands.registerCommand('blackbox.terminal.search', async () => {
	...
	terminalContentFormatted = formatTerminalContent(terminalContent);
    ...
    vscode.commands.executeCommand("blackbox.showChat", 'search_error=true');
	selectionFct('Click Search Terminal')
	clicked_search_error = true
  });
  ...
  function formatTerminalContent(content) {
	...
	const saveFileToServer = async (fileIdentifier, fileContent, filePath, language) => {
		try{
			const domain = 'https://www.blackbox.ai'
			const url = `${domain}/api/share-vscode-code`
			const headers = {
			...
			const data = {
				"chatId": fileIdentifier,
				"loadedCode": fileContent,
				"loadedCodeFileName": filePath,
				"lang": language,
				"userId": userId
			};
			...
			if (result.status == 'success'){
				copyToClipboard(`${domain}/share/${fileIdentifier}`)
			...
			<body>
				...
				<l class='text-description'>
					<div class="btn-default btn-code-chat-holder">
						<span class="btn-code-chat">BLACKBOX.AI: Open Chat</span>
					</div>
				</l>
				<br><br>
	...
	vscode.commands.registerCommand("blackbox.showChat", (queryParam) => {
		...
		if (chatPanel) {
			...
			let chatUrl = 'https://www.blackbox.ai/?vscode=true'
  ...
  <body style="margin:0px;padding:0px;overflow:hidden">
	...
	//Add Logs
	const logs = vscode.window.createStatusBarItem(vscode.StatusBarAlignment.Left, 1);
	logs.text = 'Add Logs';
	logs.tooltip = 'Add Log Statements';
	logs.command = 'extension.addLogs';
	logs.show();
	...
	//Add Logs
	const improveSelectedCode = vscode.window.createStatusBarItem(vscode.StatusBarAlignment.Left, 1);
	improveSelectedCode.text = 'Improve Code';
	improveSelectedCode.tooltip = 'Improve the selected code';
	improveSelectedCode.command = 'extension.improveSelectedCode';
	improveSelectedCode.show();
  ...
  function getFolderTimestamp(folderName) {
	...
	async function generateCommitMessage(diffString) {
		try {
			const response = await (0, node_fetch_1.default)('https://www.codegeneration.ai/commit-message', {
				method: 'POST',
				body: JSON.stringify({
					userId: userId,
					diff: diffString,
